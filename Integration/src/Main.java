...
public class Main {
	...
	public static void main(String[] args) {
		...
		/*
		 * Final keyword has a numerous way to use: A final class cannot be sub-classed.
		 * A final method cannot be overridden by subclasses. A final variable can only
		 * be initialized once.
		 */

		// String Methods: length, valueOf, equals, substring, compareTo, toUpperCase

		System.out.println("My String length, character, indexOf = " + myString.length() + ", " + myString.charAt(4)
				+ ", " + myString.indexOf("K"));

		/*
		 * In Java Programming language, String are treated as objects. The String class
		 * is immutable, so that once it is created a String object cannot be changed.
		 * For example:(The string method I used) length() = It count number of
		 * character/letter used in my String variable. charAt() = Returns the character
		 * at the specific index. indexOf() = Returns the index within this String of
		...
		class Area {
			...
			public double calculateArea(double rad) { // Here the whole line is called Header and rad is Parameter
														// double is return type and double inside () is parameter data
														// type
		...
		/*
		 * Order of Precedence: Java has well defined rules for specifying the order in
		 * which the operators in an expression are evaluated when the expressions have
		 * several operators. For example, multiplication and division have a higher
		 * precedence than addition and subtraction. Precedence rules can be overridden
		 * by explicit parentheses. When two operators share an operand the operator
		 * with higher precedence goes first.
		 * 
		 */
